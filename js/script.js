...
function generateMatrix() {
    ...
    for (i = 0; i < 8; i++) {
        $('#led-matrix').append($('<div>').addClass('row'));
        for (j = 0; j < 8; j++) {
            $('#led-matrix div.row:last').append($('<span>').addClass('cell row' + i + ' col' + j).append($('<input>').attr('type', 'checkbox').click({row: i, col: j}, setLed)));
        }
